...
export function calculateLoss(current, expected) {
    const ratio = Number(current * PERCENT_PRECISION / expected) / Number(PERCENT_PRECISION);
    return Math.abs(Math.min(ratio, 1) - 1);
}

export function createTransferBody(amount, dest, resp, forwardAmount, payload) {
    return beginCell()
...
export async function getJettonWallet(chain, owner, master) {
...
export async function getJettonBalance(chain, jettonWallet) {
...
export function allTxsOk(transactions) {
